...
from src.pipeline.prices.sbs.tipo_cambio.extract import read_raw, load_stg
...
def run(run_date: Optional[date] = None, series_override: Optional[list[dict]] = None) -> None:
    ...
    if not securities:
        logger.warning(f"No active tipo_cambio series. Exiting.")
        return
    ...
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM stg_prices_sbs_tipo_cambio WHERE reference_date = ? ORDER BY loaded_at DESC",
            (run_date.isoformat(),),
        ).fetchall()
    ...
    with get_connection() as conn:
        ...
        if not dims_df.empty and hasattr(__import__("src.pipeline.prices.sbs.tipo_cambio.loader", fromlist=["load_bond_dims"]), "load_bond_dims"):
            from src.pipeline.prices.sbs.tipo_cambio.loader import load_bond_dims
    ...
    logger.info(f"=== prices/sbs/tipo_cambio complete ===")
