# src/pipeline/prices/sbs/tipo_cambio/run.py
import logging
...
from src.pipeline.prices.sbs.tipo_cambio.extract import extract, load_stg
from src.pipeline.prices.sbs.tipo_cambio.transform import transform, FACT_FIELD_MAP
from src.pipeline.prices.sbs.tipo_cambio.loader import load_facts

logger = logging.getLogger(__name__)
TIPO_CAMBIO_FIELDS = set(FACT_FIELD_MAP.values())


def run(run_date: Optional[date] = None, series_override: Optional[list[dict]] = None) -> None:
    run_date = run_date or date.today()
    logger.info(f"=== prices/sbs/tipo_cambio | run_date={run_date} | mode={'backfill' if series_override else 'incremental'} ===")
    ...
    if series_override is not None:
        ...
        with get_connection() as conn:
            securities = [s for s in get_active_series(conn, domain="prices", source="sbs", frequency="daily") if s.get("field") in TIPO_CAMBIO_FIELDS]
    if not securities:
        logger.warning("No active tipo_cambio series. Exiting.")
        return
    securities = _classify(securities, run_date)
    to_process = [s for s in securities if s.get("start_date")]
    if not to_process:
        logger.info("All tipo_cambio series up to date. Exiting.")
        return
    ...
    with get_connection() as conn:
        rows = conn.execute("SELECT * FROM stg_prices_sbs_tipo_cambio WHERE reference_date = ? ORDER BY loaded_at DESC", (run_date.isoformat(),)).fetchall()
        stg_df = pd.DataFrame([dict(r) for r in rows]) if rows else pd.DataFrame()
    facts_df = transform(stg_df, to_process)
    if facts_df.empty:
    ...
    logger.info("=== prices/sbs/tipo_cambio complete ===")
