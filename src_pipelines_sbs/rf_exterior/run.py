...
from src.pipeline.prices.sbs.rf_exterior.extract import extract, load_stg
from src.pipeline.prices.sbs.rf_exterior.transform import transform, FACT_FIELD_MAP
from src.pipeline.prices.sbs.rf_exterior.loader import load_facts, load_bond_dims

logger = logging.getLogger(__name__)
RF_EXTERIOR_FIELDS = set(FACT_FIELD_MAP.values())
...
def run(run_date: Optional[date] = None, series_override: Optional[list[dict]] = None) -> None:
    ...
    if series_override is not None:
        ...
        with get_connection() as conn:
            securities = [s for s in get_active_series(conn, domain="prices", source="sbs", frequency="daily") if s.get("field") in RF_EXTERIOR_FIELDS]
    if not securities:
    ...
    with get_connection() as conn:
        rows = conn.execute("SELECT * FROM stg_prices_sbs_rf_exterior WHERE reference_date = ? ORDER BY loaded_at DESC", (run_date.isoformat(),)).fetchall()
