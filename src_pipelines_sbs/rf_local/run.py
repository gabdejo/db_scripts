# src/pipeline/prices/sbs/rf_local/run.py
import logging
...
from src.pipeline.prices.sbs.rf_local.extract import extract, load_stg
from src.pipeline.prices.sbs.rf_local.transform import transform, FACT_FIELD_MAP
from src.pipeline.prices.sbs.rf_local.loader import load_facts, load_bond_dims

logger = logging.getLogger(__name__)
RF_LOCAL_FIELDS = set(FACT_FIELD_MAP.values())


def run(run_date: Optional[date] = None, series_override: Optional[list[dict]] = None) -> None:
    run_date = run_date or date.today()
    logger.info(f"=== prices/sbs/rf_local | run_date={run_date} | mode={'backfill' if series_override else 'incremental'} ===")
    ...
    if series_override is not None:
        ...
        with get_connection() as conn:
            securities = [s for s in get_active_series(conn, domain="prices", source="sbs", frequency="daily") if s.get("field") in RF_LOCAL_FIELDS]
    if not securities:
        logger.warning("No active rf_local series. Exiting.")
        return
    securities = _classify(securities, run_date)
    to_process = [s for s in securities if s.get("start_date")]
    if not to_process:
        logger.info("All rf_local series up to date. Exiting.")
        return
    ...
    with get_connection() as conn:
        rows = conn.execute("SELECT * FROM stg_prices_sbs_rf_local WHERE reference_date = ? ORDER BY loaded_at DESC", (run_date.isoformat(),)).fetchall()
    ...
    logger.info("=== prices/sbs/rf_local complete ===")
