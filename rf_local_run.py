...
from src.pipeline.prices.sbs.rf_local.extract import read_raw, load_stg
from src.pipeline.prices.sbs.rf_local.transform import transform, FACT_FIELD_MAP
from src.pipeline.prices.sbs.rf_local.loader import load_facts
...
def run(run_date: Optional[date] = None, series_override: Optional[list[dict]] = None) -> None:
    ...
    if not securities:
        logger.warning(f"No active rf_local series. Exiting.")
        return
    ...
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM stg_prices_sbs_rf_local WHERE reference_date = ? ORDER BY loaded_at DESC",
            (run_date.isoformat(),),
        ).fetchall()
    ...
    with get_connection() as conn:
        ...
        if not dims_df.empty and hasattr(__import__("src.pipeline.prices.sbs.rf_local.loader", fromlist=["load_bond_dims"]), "load_bond_dims"):
            from src.pipeline.prices.sbs.rf_local.loader import load_bond_dims
    ...
    logger.info(f"=== prices/sbs/rf_local complete ===")
