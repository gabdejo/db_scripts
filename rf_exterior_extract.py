# src/pipeline/prices/sbs/rf_exterior/extract.py
import logging
...
SBS_SUBDIR = "rf_exterior"

RAW_COLUMNS = {
    "codigo_sbs": "codigo_sbs", "isin": "isin",
    ...
    "interes_corrido_monto": "interes_corrido_monto",
    "fecha_emision": "fecha_emision", "fecha_vencimiento": "fecha_vencimiento",
    "tasa_cupon": "tasa_cupon", "ultimo_cupon": "ultimo_cupon",
    "proximo_cupon": "proximo_cupon", "variacion_precio_sucio": "variacion_precio_sucio",
}
    ...
    for _, row in df.iterrows():
        conn.execute(
            ...
            INSERT INTO stg_prices_sbs_rf_exterior (
                ...
                precio_sucio_pct, interes_corrido_monto, variacion_precio_sucio,
                reference_date, loaded_at
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ON CONFLICT (codigo_sbs, reference_date, loaded_at) DO NOTHING
            """,
            (
                _s(row,"codigo_sbs"), _s(row,"isin"), _s(row,"tipo_instrumento"),
                _s(row,"emisor"), _s(row,"moneda"), _f(row.get("valor_facial")),
                _s(row,"origen_precio"), _s(row,"fecha_emision"), _s(row,"fecha_vencimiento"),
                _f(row.get("tasa_cupon")), _s(row,"ultimo_cupon"), _s(row,"proximo_cupon"),
                _f(row.get("precio_limpio_monto")), _f(row.get("precio_limpio_pct")),
                _f(row.get("precio_sucio_monto")), _f(row.get("precio_sucio_pct")),
                _f(row.get("interes_corrido_monto")), _f(row.get("variacion_precio_sucio")),
